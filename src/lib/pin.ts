export async function hashPin(pin: string): Promise<string> {
  ...
  // Over LAN HTTP (e.g. phone testing via 192.168.x.x), use pure JS SHA-256.
...
// Pure JS SHA-256 — produces identical output to crypto.subtle.digest('SHA-256')
function sha256(message: Uint8Array): Uint8Array {
  ...
  const padLen = (((message.length + 8) >> 6) + 1) << 6;
